...
FILEHANDLER_PARALLEL_DEFAULT = config['analysis'].getboolean(
    'FILEHANDLER_PARALLEL_DEFAULT')
FILEHANDLER_TOUCH_TMPFILE = config['analysis'].getboolean(
...
def add_file_handler_parareal(evaluator, filename, set_num_in, **kw):
    ...
    """
    FH = FileHandlerParareal(filename, evaluator.domain, evaluator.vars,
                             set_num_in=set_num_in, **kw)
...
def split_comms(dedalus_slices):
    ...
    assert world_size % dedalus_slices == 0, "total processors not equal to " \
                                             "space x time"
    ...
    x_comm = world.Split(t_number, key)  # create communicator for spatial
    # parallelisation (dedalus)
...
class Parareal_solver:
    ...
    def __init__(self, fine_solver, coarse_dt, fine_dt, ratio, T_end, t_comm,
                 file_name):
        ...
        self.communication_fields = self.fine_solver.problem.variables  #
        # list of strings
    ...
    def get_solver_details(self, solver):
        ...
        for i in range(len(solver.problem.domain.bases)):
            ...
            # print(base_N)
            base_list.append({'name': base_name, 'interval': interval,
    ...
    def set_up_coarse(self):
        ...
        for i in range(len(base_list)):
            ...
            coarse_res = np.rint(
                base_list[i]['N'] * (1 / (self.ratio))).astype(int)
            if base_type == 'Fourier':
                base = de.Fourier(base_list[i]['name'], coarse_res,
                                  interval=base_list[i]['interval'],
                                  dealias=base_list[i]['dealias'])
                coarse_bases.append(base)
            elif base_type == 'Chebyshev':
                coarse_bases.append(
                    de.Chebyshev(base_list[i]['name'], coarse_res,
                                 interval=base_list[i]['interval'],
                                 dealias=base_list[i]['dealias']))
            elif base_type == 'SinCos':
                coarse_bases.append(de.SinCos(base_list[i]['name'], coarse_res,
                                              interval=base_list[i][
                                                  'interval'],
                                              dealias=base_list[i]['dealias']))
        ...
        for var in self.communication_fields:
            for base in base_list:
                base_name = base['name']
                meta_keys = self.fine_solver.problem.meta[var][
                    base_name].keys()
    ...
    def setUpStorage(self):
        ...
        for i in range(len(self.communication_fields)):
            self.G_n1_k.append(np.empty_like(
                self.fine_solver.state[self.communication_fields[0]]['g']))
            self.F_n1_k.append(np.empty_like(
                self.fine_solver.state[self.communication_fields[0]]['g']))
            self.G_n1_k1.append(np.empty_like(
                self.fine_solver.state[self.communication_fields[0]]['g']))
            self.correction_k0.append(np.empty_like(
                self.fine_solver.state[self.communication_fields[0]]['g']))
            self.correction_k1.append(np.empty_like(
                self.fine_solver.state[self.communication_fields[0]]['g']))
            self.temp.append(np.empty_like(
                self.fine_solver.state[self.communication_fields[0]]['g']))

    def getGridIndices(self):
        self.dims = []
        for dimension in range(self.fine_domain.dim):
            L = self.fine_domain.bases[dimension].interval[1] - \
                self.fine_domain.bases[dimension].interval[0]
            N = self.fine_domain.global_grid_shape()[dimension]
            self.dims.append(np.rint(
                np.array(self.fine_domain.grid(dimension)) / L * N).astype(
                int))
            self.dims[dimension] = np.reshape(self.dims[dimension],
                                              np.size(self.dims[dimension]))
        return self.dims
    ...
    def setInitial_coarse(self):
        if self.t_rank == 0:
            ...
            for field in self.communication_fields:
                ...
                self.coarse_solver.state[field]['g'] = np.copy(
                    self.initial_conditions[field_counter])
    ...
    def set_up_slices(self):
        ...
        print("processor:{}, start time:{}".format(self.t_comm.rank,
                                                   self.slice_start))
        self.slice_end = (self.t_comm.rank + 1) * self.slice_size
        self.N_coarse_steps = np.rint(self.slice_size / self.coarse_dt).astype(
            int)
        ...
        if True:  # self.t_rank==self.t_size-1:
            ...
            for i in range(len(self.communication_fields)):
                error = np.linalg.norm(
                    self.correction_k0[i] - self.correction_k1[
                        i]) / np.linalg.norm(self.correction_k1[i])
    ...
    def coarse_initial_run(self):
        ...
        if self.t_rank != 0:
            for i in range(len(self.communication_fields)):
                ...
                self.coarse_solver.state[field].set_scales(self.ratio)
                self.t_comm.Recv(self.coarse_solver.state[field]['g'],
                                 source=self.t_rank - 1, tag=10 + i)
                self.coarse_solver.state[field].set_scales(self.ratio)
                # put initial conditions from previous time slice on fine
                # solver
                self.fine_solver.state[field].set_scales(1)
                self.fine_solver.state[field]['g'] = np.copy(
        ...
        while not written:
            try:
                ...
                print(e)
                time.sleep(2)
                print("ERROR p_{}, k_{}, failed reset analysis coarse".format(
                    self.t_comm.rank, self.k))
        ...
        if self.t_rank != self.t_size - 1:
            for i in range(len(self.communication_fields)):
                field = self.communication_fields[i]
                self.coarse_solver.state[field].set_scales(self.ratio)
                self.t_comm.Send(self.coarse_solver.state[field]['g'],
                                 dest=self.t_rank + 1, tag=10 + i)
        ...
        if self.x_comm.rank == 0:
            print(
                "Time slice:{} of {}, completed coarse run".format(self.t_rank,
                                                                   self.t_comm.size))
    ...
    def parareal_iteration(self):
        ...
        if self.t_rank != 0:
            for i in range(len(self.communication_fields)):  # iterate
                # through all dedalus fields
                field = self.communication_fields[i]

                self.coarse_solver.state[field].set_scales(self.ratio)
                self.t_comm.Recv(self.coarse_solver.state[field]['g'],
                                 source=self.t_rank - 1, tag=20 + i)

                # copy initial condition to fine solver ready for next
                # iteration
                self.fine_solver.state[field].set_scales(1)
                self.fine_solver.state[field]['g'] = np.copy(
                    self.coarse_solver.state[field]['g'])
        ...
        for index in range(len(self.communication_fields)):
            ...
            self.correction_k1[index] = np.copy(self.G_n1_k1[index]) + np.copy(
                self.F_n1_k[index]) - np.copy(self.G_n1_k[index])

            if self.t_rank != self.t_size - 1:
                self.t_comm.Send(self.correction_k1[index],
                                 dest=self.t_rank + 1, tag=20 + index)
    ...
    def variable_stepping_fine(self):
        ...
        while self.fine_solver.ok:
            ...
            if (
                    self.fine_solver.stop_sim_time -
                    self.fine_solver.sim_time) < 1.1 * dt:
    ...
    def variable_stepping_coarse(self):
        ...
        while self.coarse_solver.ok:
            ...
            if (
                    self.coarse_solver.stop_sim_time -
                    self.coarse_solver.sim_time) < 1.1 * dt:
                dt = self.coarse_solver.stop_sim_time - \
                     self.coarse_solver.sim_time
    ...
    def save_state(self):
        ...
        """
        self.x_comm.Barrier()
        file_name = self.base_name + "/" + self.base_name.split("/")[
            -1] + "_k_{}".format(self.k)
        mode = 'specify'
        snapshots = add_file_handler_parareal(self.fine_solver.evaluator,
                                              file_name, iter=1,
                                              max_writes=np.inf, mode=mode,
                                              set_num_in=self.t_comm.rank)
    ...
    def save_state_final(self):
        ...
        """
        self.x_comm.Barrier()
        file_name = self.base_name + "/" + self.base_name + "_k_{}".format(
            self.k)
        mode = 'specify'
        snapshots = add_file_handler_parareal(self.fine_solver.evaluator,
                                              file_name, iter=1,
                                              max_writes=np.inf, mode=mode,
                                              set_num_in=self.t_comm.rank + 1)
        for handler in self.fine_solver.evaluator.handlers:
            # print(handler)
    ...
    def add_file_handler(self, file_name, iter, max_writes, mode='specify'):
        ...
        self.my_handler = {"file_path": analysis_name, "iter_number": iter,
                           "max_write": max_writes, "mode": 'specify'}
    ...
    def add_task(self, task_string, layout='g', name='default', scales=1):
        ...
        write up.
        """
        self.tasks.append({"task": task_string, "layout": layout, "name": name,
                           "scales": scales})
    ...
    def reset_analysis(self):
        ...
        analysis_name = self.analysis_name + "/" + \
                        self.analysis_name.split("/")[-1] + "_k_{}".format(
            self.k)
        if self.k > 1:
            self.system_analysis.iter = np.inf
        ...
        while not worked:
            try:
                self.system_analysis = add_file_handler_parareal(
                    self.fine_solver.evaluator, analysis_name, iter=iters,
                    max_writes=max_writes, mode=mode,
                    set_num_in=self.t_comm.rank)
                ...
                print(e)
                print(
                    "t rank:{}, x rank:{}, k:{}, fine reset analysis failed, "
                    "retrying...X".format(
                        self.t_comm.rank, self.x_comm.rank, self.k))
                print("File name: ", analysis_name)
                time.sleep(1)

        for i in range(len(self.tasks)):
            my_dict = self.tasks[i]
            self.system_analysis.add_task(my_dict["task"],
                                          layout=my_dict["layout"],
                                          scales=my_dict["scales"],
                                          name=my_dict["name"])
    ...
    def reset_analysis_coarse(self):
        ...
        while not worked:
            try:
                # self.system_analysis_coarse=self.coarse_solver.evaluator
                # .add_file_handler(analysis_name,iter=iters,
                # max_writes=max_writes,mode=mode)
                self.system_analysis_coarse = add_file_handler_parareal(
                    self.coarse_solver.evaluator, analysis_name, iter=iters,
                    max_writes=max_writes, mode=mode,
                    set_num_in=self.t_comm.rank)
                ...
                print(e)
                print(
                    "t rank:{}, x rank:{}, k:{}, coarse reset analysis "
        ...
        for i in range(len(self.tasks)):
            my_dict = self.tasks[i]
            self.system_analysis_coarse.add_task(my_dict["task"],
                                                 layout=my_dict["layout"],
                                                 scales=my_dict["scales"],
                                                 name=my_dict["name"])
    ...
    def plot_dirty(self):
        ...
        if self.t_rank == self.t_size - 1:
            ...
            plt.plot(np.linspace(0, 1, len(self.correction_k1[0][0, :])),
                     self.correction_k1[0][0, :], label=self.k)
            plt.legend()
            plt.pause(0.1)
...
class FileHandlerParareal(FileHandler):
    ...
    def __init__(self, base_path, *args, max_writes=np.inf, max_size=2 ** 30,
                 parallel=None, mode=None, set_num_in, **kw):
        ...
        if comm.rank == 0:
            ...
            if mode == "overwrite":
                ...
                if sets:
                    ...
                    joined_file = base_path.joinpath(
                        "{}_s{}.h5".format(base_path.stem, max_set))
                    p0_file = base_path.joinpath(
                        "{0}_s{1}/{0}_s{1}_p0.h5".format(base_path.stem,
                                                         max_set))
